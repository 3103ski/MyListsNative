...
const list = (
	...
	switch (action.type) {
		...
		case a.FETCH_LISTS_START:
			return updateObject(state, { isLoadingLists: true });
		case a.FETCH_LISTS_ERROR:
			return updateObject(state, { isLoadingLists: false, errorMsg: action.errorMsg });
		case a.FETCH_LISTS_SUCCESS:
			return updateObject(state, { isLoadingLists: false, lists: action.lists });
